...
pub(crate) fn solve_lwe(
    ...
    let s_pred = enumerate_approximate(&s_pred, n, q, A, b);
...
fn enumerate_approximate(
    ...
    fn dfs(
        ...
        if i == n {
            let err = compute_error(A, current, b, q);
            if err < *best_err {
                *best_err = err;
                *best = current.clone();
            }
        ...
        for delta in [-1i64, 0, 1] {
            ...
            current[i] = new_val;
            dfs(i + 1, n, current, best, best_err, A, b, q);
        }
        current[i] = orig;
    }

    dfs(0, n, &mut current, &mut best, &mut best_err, A, b, q);
    best
}



fn compute_error(A: &Array2<u64>, s: &Array1<u64>, b: &Array1<u64>, q: u64) -> f64 {
    let As = A.t().to_owned().dot(s);
    As.iter()
        .zip(b.iter())
        .map(|(&a, &b_i)| {
            let diff = ((a as i64 - b_i as i64).rem_euclid(q as i64)) as f64;
            diff * diff
        })
        .sum::<f64>()
        .sqrt()
