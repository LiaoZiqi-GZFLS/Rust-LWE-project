...
fn main() {
    ...
    for (i, &(n, m, q, alpha, cur_score)) in test_cases.iter().enumerate() {
        ...
        let error1 = solver::compute_error(&instance.a.to_owned(), &instance.s.mapv(|x| x % q), &instance.b, q);
        let error2 = solver::compute_error(&instance.a.to_owned(), &s_pred.mapv(|x| x % q), &instance.b, q);
        println!("Error1: {:.4}, Error2: {:.4}", error1, error2);
